...
// kobu: bounded restart-from-scratch retries when the flaky 3-wire SPI read
// makes the self-test/product-id step fail at power-up (see pmw3610_async_init).
...
static void pmw3610_async_init(struct k_work *work) {
    ...
    if (data->err) {
        ...
        // kobu: the 3-wire SPI read is flaky at power-up on this board and can
        // return 0xFF garbage, failing the self-test/product-id step. Rather
        // than give up (which left the scroll ball dead until a reboot),
        // restart init from scratch a bounded number of times.
        if (data->init_retries < PMW3610_INIT_MAX_RETRIES) {
            data->init_retries++;
            data->async_init_step = 0;
            data->err = 0;
            LOG_WRN("PMW3610 init retry %d/%d", data->init_retries, PMW3610_INIT_MAX_RETRIES);
            k_work_schedule(&data->init_work, K_MSEC(PMW3610_INIT_RETRY_DELAY_MS));
        } else {
            LOG_ERR("PMW3610 init gave up after %d retries", data->init_retries);
        }
    } else {
...
#define TOINT16(val, bits) (((struct { int16_t value : bits; }){val}).value)
    ...
    // is a reliable garbage signature. Without this, 0xFF both sets the MOT bit
    // (bit7) AND decodes to nonzero x/y -> PHANTOM scroll. This REPLACES the
    // earlier MOT-bit gate, which was the wrong filter: in force-awake polling
    // the MOT bit is unreliable for genuine (esp. slow) motion, while a truly
    // idle sensor already returns x=y=0 (confirmed by USB-log capture), so no
    // MOT gate is needed once garbage reads are dropped.
    if (buf[0] == 0xFF) {
        dx = 0;
        dy = 0;
        return 0;
...
static void pmw3610_poll_work_callback(struct k_work *work) {
    struct k_work_delayable *work2 = (struct k_work_delayable *)work;
    struct pixart_data *data = CONTAINER_OF(work2, struct pixart_data, poll_work);
    const struct device *dev = data->dev;
...
static int on_activity_state(const zmk_event_t *eh) {
    ...
    // kobu: keep force-awake sensors PINNED in RUN mode across activity changes,
    // matching the RMK firmware (rmk 0.8 pmw3610.rs sets force_awake once at init
    // and never downshifts). pmw3610_set_performance() is already a no-op for
    // non-force-awake sensors, so the only effect of the original
    // `ACTIVE ? 1 : 0` was to drop the force-awake (scroll/pointer) balls into
    // NORMAL on idle, letting them REST. A rested, re-awakening sensor emits
    // noisier idle samples that can carry the MOTION bit and leak phantom scroll
    // ("暴発") past the MOT gate. Re-assert awake on every transition instead.
    (void)state_ev;
